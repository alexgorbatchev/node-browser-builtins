...
test('stream - unshift read race', function (tt) {
  var hwm = 10;
  ...
  r._read = function(n) {
    tt.ok(!pushedNull, '_read after null push');

    // every third chunk is fast
    push(!(chunks % 3));

    function push(fast) {
      tt.ok(!pushedNull, 'push() after null push');
  ...
  var ended = false;
  r.on('end', function() {
    tt.ok(!ended, 'end emitted more than once');
    tt.throws(function() {
  ...
  w.on('finish', function() {
    ...
    // lacking that piece.
    tt.equal(written[0], 'asdfasdfas');
    var asdf = 'd';
    for (var i = 1; i < written.length; i++) {
      tt.equal(written[i].slice(0, 4), '1234');
  ...
  function done() {
    tt.equal(written.length, 18);
    tt.ok(ended, 'stream ended');
    tt.ok(finished, 'stream finished');
    tt.end();
  }
  w.on('finish', done);
});
